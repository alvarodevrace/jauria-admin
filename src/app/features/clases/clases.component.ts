...
import { format, startOfWeek, addDays, addWeeks, subWeeks, isSameDay, isWithinInterval, parseISO, differenceInMinutes } from 'date-fns';
...
@Component({
  ...
  template: `
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;margin-bottom:20px;">
      ...
      <div style="display:flex;align-items:center;gap:12px;">
        <button class="btn btn--ghost btn--sm" (click)="semanaAnterior()">‹</button>
        <span style="font-family:'Bebas Neue',sans-serif;font-size:16px;letter-spacing:0.05em;color:#fff;">
          {{ semanaLabel() }}
        </span>
        <button class="btn btn--ghost btn--sm" (click)="semanaSiguiente()">›</button>
        <button class="btn btn--ghost btn--sm" (click)="irHoy()">Hoy</button>
      </div>
      <!-- Vista + acciones -->
      <div style="display:flex;gap:10px;align-items:center;">
        <div style="display:flex;border:1px solid #2a2a2a;border-radius:8px;overflow:hidden;">
          <button class="btn btn--sm" [class]="vista()==='semana' ? 'btn--primary' : 'btn--ghost'" style="border-radius:0;" (click)="vista.set('semana')">Semana</button>
          <button class="btn btn--sm" [class]="vista()==='lista' ? 'btn--primary' : 'btn--ghost'" style="border-radius:0;" (click)="vista.set('lista')">Lista</button>
        </div>
        @if (auth.isCoach()) {
          <button class="btn btn--primary btn--sm" (click)="abrirFormClase()">+ Nueva Clase</button>
        }
      </div>
    </div>
        ...
        <div class="week-calendar__header">
          ...
          @for (dia of diasSemana(); track dia.fecha) {
            <div class="week-calendar__day-header" [class.today]="dia.esHoy">
              ...
              <div class="week-calendar__day-num" [class.today]="dia.esHoy">{{ dia.num }}</div>
            </div>
          }
        </div>
        <!-- Grid -->
          ...
          @for (dia of diasSemana(); track dia.fecha) {
            <div class="week-calendar__col" [class.today]="dia.esHoy">
              @for (clase of clasesDelDia(dia.fecha); track clase.id) {
                ...
                >
                  <div class="week-event__time">{{ clase.hora_inicio }} – {{ clase.hora_fin }}</div>
                  <div class="week-event__title">{{ clase.tipo }}</div>
                  @if (clase.descripcion) {
                    <div class="week-event__desc">{{ clase.descripcion }}</div>
                  }
                  <div class="week-event__coach">{{ clase.profiles?.nombre_completo ?? '—' }}</div>
                </div>
              }
    ...
    @if (vista() === 'lista') {
      <div class="data-table-wrapper">
        <div class="data-table-wrapper__header">
          ...
          <select class="form-control" style="width:auto;height:38px;" [(ngModel)]="filterTipo" (change)="cargarClases()">
            ...
            <tbody>
              @for (c of clases(); track c.id) {
                <tr [class.row-cancelled]="c.cancelada">
                  <td>
                    <span [style.background]="tipoBg(c.tipo)" [style.color]="tipoColor(c.tipo)"
                          style="padding:3px 10px;border-radius:20px;font-size:11px;font-weight:700;">
                      {{ c.tipo }}
                    </span>
                  </td>
                  <td style="font-size:13px;">{{ c.fecha | dateEc : 'EEEE dd/MM' }}</td>
                  <td style="font-size:13px;">{{ c.hora_inicio }} – {{ c.hora_fin }}</td>
                  <td style="font-size:13px;">{{ c.profiles?.nombre_completo ?? '—' }}</td>
                  ...
                  <td>
                    <div class="data-table__actions">
                      ...
                      @if (auth.isCoach()) {
                        <button class="btn btn--danger btn--sm" (click)="cancelarClase(c)" [disabled]="c.cancelada">
                ...
                <tr><td colspan="6" style="text-align:center;padding:40px;color:#666;">Sin clases esta semana.</td></tr>
              }
            </tbody>
          </table>
        }
      ...
      <div class="modal-backdrop" (click)="showFormClase.set(false)">
        <div class="modal modal--wide" (click)="$event.stopPropagation()">
          <div class="modal__header">
            ...
            <button class="btn btn--ghost btn--icon" (click)="showFormClase.set(false)">✕</button>
          </div>
            ...
            <form (ngSubmit)="crearClase()">
              <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                <div class="form-group">
                  ...
                  <select class="form-control" [(ngModel)]="newClase.tipo" name="tipo" required>
                ...
                <div class="form-group">
                  ...
                  <input class="form-control" type="date" [(ngModel)]="newClase.fecha" name="fecha" required />
                </div>
                <div class="form-group">
                  <label class="form-label">Hora Inicio *</label>
                  <input class="form-control" type="time" [(ngModel)]="newClase.hora_inicio" name="hinicio" required />
                </div>
                <div class="form-group">
                  <label class="form-label">Hora Fin *</label>
                  <input class="form-control" type="time" [(ngModel)]="newClase.hora_fin" name="hfin" required />
                </div>
                <div class="form-group">
                  <label class="form-label">Capacidad Máx.</label>
                  <input class="form-control" type="number" [(ngModel)]="newClase.capacidad_maxima" name="cap" min="1" max="100" />
                </div>
                <div class="form-group" style="grid-column:1/-1;">
                  <label class="form-label">Descripción / WOD del día</label>
                  <input class="form-control" type="text" [(ngModel)]="newClase.descripcion" name="desc" placeholder="Ej: 3 rounds: 10 thrusters, 10 pull-ups..." />
                </div>
              </div>
              <div class="modal__footer" style="padding:0;margin-top:20px;">
                <button type="button" class="btn btn--ghost" (click)="showFormClase.set(false)">Cancelar</button>
                <button type="submit" class="btn btn--primary" [disabled]="savingClase()">
        ...
        <div class="modal modal--wide" (click)="$event.stopPropagation()">
          <div class="modal__header">
            <h3 class="modal__title">
              ...
              <span style="font-size:14px;font-weight:400;color:#666;text-transform:none;font-family:'Inter',sans-serif;">
                · {{ claseSeleccionada()!.fecha | dateEc : 'EEEE dd/MM' }}
                {{ claseSeleccionada()!.hora_inicio }} – {{ claseSeleccionada()!.hora_fin }}
              </span>
            </h3>
            <button class="btn btn--ghost btn--icon" (click)="claseSeleccionada.set(null)">✕</button>
          </div>
          <div class="modal__body">
            ...
            @if (inscritosLoading()) {
              <div style="text-align:center;padding:24px;color:#666;">Cargando inscritos...</div>
            } @else if (inscritos().length === 0) {
              <div style="text-align:center;padding:24px;color:#666;">Sin inscritos todavía.</div>
            } @else {
              <table class="data-table">
                <thead>
                  <tr>
                    <th>Atleta</th>
                    <th>Estado</th>
                    @if (auth.isCoach()) { <th>Asistencia</th> }
                  </tr>
                </thead>
                <tbody>
                  @for (ins of inscritos(); track ins.id) {
                    <tr>
                      <td style="font-weight:600;color:#fff;">{{ ins.profiles?.nombre_completo ?? '—' }}</td>
                      <td><span class="badge badge--{{ inscripcionBadge(ins.estado) }}">{{ ins.estado }}</span></td>
                      @if (auth.isCoach()) {
                        <td>
                          <div style="display:flex;gap:8px;">
                            <button class="btn btn--sm btn--ghost" (click)="marcarAsistencia(ins.id, true)" [disabled]="ins.estado === 'asistio'" title="Asistió">✓ Asistió</button>
                            <button class="btn btn--sm btn--danger" (click)="marcarAsistencia(ins.id, false)" [disabled]="ins.estado === 'no_asistio'" title="No asistió">✕ No asistió</button>
                          </div>
                        </td>
                      }
                    </tr>
  ...
  styles: [`
    ...
    .week-calendar__day-header {
      ...
      text-align: center;
      border-right: 1px solid #1e1e1e;
      &.today { background: rgba(183,28,28,0.08); }
    }
    ...
    .week-calendar__col {
      ...
      gap: 6px;
      &.today { background: rgba(183,28,28,0.04); }
      &:last-child { border-right: none; }
    }
    .week-calendar__empty { flex: 1; }
    .week-event {
      border-radius: 8px;
      padding: 8px 10px;
      cursor: pointer;
    ...
    .week-event--wod         { background: rgba(183,28,28,0.25); border-left: 3px solid #B71C1C; }
    .week-event--open-gym    { background: rgba(80,80,80,0.3); border-left: 3px solid #888; }
    .week-event--barbell-club { background: rgba(123,17,17,0.3); border-left: 3px solid #7B1111; }
    .week-event--fundamentos { background: rgba(33,150,243,0.2); border-left: 3px solid #42a5f5; }
...
export class ClasesComponent implements OnInit {
  ...
  semanaSiguiente() { this.semanaBase.update(d => addWeeks(d, 1)); this.cargarClases(); }
  irHoy() { this.semanaBase.set(startOfWeek(new Date(), { weekStartsOn: 1 })); this.cargarClases(); }

  async cargarClases() {
    this.loading.set(true);
    const lunes = format(this.semanaBase(), 'yyyy-MM-dd');
    const domingo = format(addDays(this.semanaBase(), 6), 'yyyy-MM-dd');
    const { data, error } = await this.supabase.getClases({ semana: `${lunes},${domingo}` });
    this.loading.set(false);
    if (error) {
      this.sentry.captureError(error, { action: 'cargarClases', semana: lunes });
  ...
  async cargarMisInscripciones() {
    ...
    this.misInscritas.set(
      ((data ?? []) as Record<string, unknown>[]).map(i => i['clase_id'] as number)
    );
  }
  ...
  async crearClase() {
    ...
    const { error } = await this.supabase.createClase({ ...this.newClase, coach_id: coachId });
  ...
  async inscribirseOCancelar(clase: Clase) {
    ...
    if (this.inscritoEn(clase.id)) {
      ...
      if (error?.code === '23505') { this.toast.warning('Ya estás inscrito'); }
      else if (error) { this.toast.error(error.message); }
      else {
        this.misInscritas.update(ids => [...ids, clase.id]);
        this.toast.success('¡Inscripción confirmada!');
        this.sentry.addBreadcrumb(`Inscripción clase ${clase.tipo} ${clase.fecha}`, 'clases');
      }
    }
  ...
  async marcarAsistencia(inscripcionId: number, asistio: boolean) {
    const { error } = await this.supabase.marcarAsistencia(inscripcionId, asistio);
    if (error) { this.toast.error(error.message); return; }
    this.inscritos.update(list => list.map(i =>
      i.id === inscripcionId ? { ...i, estado: asistio ? 'asistio' : 'no_asistio' } : i
    ));
    this.toast.success(asistio ? 'Asistencia confirmada' : 'Marcado como ausente');
  }

  async cancelarClase(clase: Clase) {
    const motivo = prompt('Motivo de cancelación (opcional):') ?? '';
    const { error } = await this.supabase.updateClase(clase.id, { cancelada: true, motivo_cancelacion: motivo });
    if (error) { this.toast.error(error.message); return; }
  ...
  inscripcionBadge(estado: string): string {
    return ({ inscrito: 'esperando', asistio: 'activo', no_asistio: 'vencido', cancelado: 'inactivo' } as Record<string, string>)[estado] ?? 'inactivo';
  }

  tipoBg(tipo: string): string {
    return ({ WOD: 'rgba(183,28,28,0.2)', 'Open Gym': 'rgba(80,80,80,0.2)', 'Barbell Club': 'rgba(123,17,17,0.2)', Fundamentos: 'rgba(33,150,243,0.2)' } as Record<string, string>)[tipo] ?? 'rgba(80,80,80,0.2)';
  }

  tipoColor(tipo: string): string {
    return ({ WOD: '#B71C1C', 'Open Gym': '#CCCCCC', 'Barbell Club': '#7B1111', Fundamentos: '#42a5f5' } as Record<string, string>)[tipo] ?? '#aaa';
  }
  ...
  private emptyClase() {
    return {
      tipo: '', fecha: format(new Date(), 'yyyy-MM-dd'),
      hora_inicio: '06:00', hora_fin: '07:00',
      capacidad_maxima: 20, descripcion: '',
    };
  }
}
